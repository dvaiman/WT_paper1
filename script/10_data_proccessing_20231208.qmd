...
df1 |> 
 ...
 # status() %>% 
  group_by(test_count>1, .headline = "Stratified by if having 1 or more tests  ") %>% 
...
Table 1 is to describe the study population. These variables will not be used anywhere else in this manuscript. However it shows the percentage missing in the different variables. 
```{r}
...
Sociodemographic variables at the time of the HPI test are displayed as percentages across various groupings, serving both to characterize the study population and to lay the groundwork for subsequent analyses. These percentages will be further examined in comparison with SCB data in the upcoming figure and table.
```{r}
...
Figure shows percentage point differences for year groups and total in different sociodemographic categories. THe figure is not in its final form, more of a concept image that will be improved. 




<span style="color:red;">**EJ RIKTIG DATA** väntar fortfarande på jämförelsedata från SCB. Exempeldata.</span> R.
...
<span style="color:red;">**EJ RIKTIG DATA** väntar fortfarande på jämförelsedata från SCB. Exempeldata.</span>
```{r}
 rel_diff_table <- 
   compare_percentages(
  df1 = df2 %>% filter(test_count == 1),
...
This part is missing a lot of data as we are waiting for register data from statistics Sweden. However it is meant that it should follow same pattern as the presentation of socioeconomic data above. below we will present some possible aggregations of the long awaited SCB data. The categorization is up to discussion.  
...
<span style="color:red;">Denna Väntar på jämförelsedata från SCB..</span> 
...
Figure 3 will be the pinnacle of this manuscript. it will not only show over represented and underrepresented groups in HPI data compared to the whole working population of Sweden, but it will show groups within groups within groups. However the **HOW** is yet to be decided. The figure is also awaiting SCB data.  
  
First step is to choose variables to be used in the aggregation. A suggestion is to keep it simple with **occupation, sex, age and by ownership sector**, and keep the data more up to date - say 2017-2023. But it could also depend on the method used to find over and underrepresented groups. Other variables of interest could be **year period, education level, work sector, company size, geographic region, country of birth and operating profit**.  
  
One way is to choose the 20 most underrepresented and the 20 most overrepresented groups based on their deviation from the expected representation. This could be shown in a bar chart with groups on the x-axis and their deviation from expected representation on the y-axis. Another method involves using hierarchical clustering to find and present groups with similar representation patterns. A vizual example of this can be seen below. The dendrogram visualizes the hierarchical clustering of the groups based on their features. Each branch represents a cluster, and the height of the branches indicates the distance (or dissimilarity) between clusters. Observations (or groups) that are more similar to each other based on the features are clustered together and joined at lower points on the dendrogram. By selecting a specific level of dissimilarity from the dendrogram figure, we identify distinct clusters. These clusters can subsequently be illustrated graphically to visualize the grouping. Other suggestions are welcome here. 
...
https://www.sjweh.fi/show_abstract.php?abstract_id=3928&fullText=1#box-fullText
...
 
Number of employees in the company (public/private???) (Ast_AntalSys)
...
Multiple jobs/ economic sectors	(AntAns, SNI)
1. ≥2 jobs in >1 sector	
2. ≥2 jobs	
3. 1 job

Staff Turnover Rate (%) (PersOms_Harledd / Ast_AntalSys) 
1. (number of separations during the period / Average number of employees under the period) x 100

Contractual relationship insecurity	(vi kanske inte har denna data?)
1. Solo self-employed	
2. Self- and direct employment / Self-employed / Agency employed	
3. Directly employed
...
Net sales % av median? (Ftg_Nettoomsattning)
≥200%
120–199%
80–119%
60–79
...
Operating Profit (Ftg_Rorelseresultat)
≥200%
120–199%
80–119%
60–79
...
Operating Margin (Ftg_Nettoomsattning, Ftg_Rorelseresultat)
1. (operating profit/revenue)x100


Collective bargaining agreement coverage (% likelihood)	(kanske tas bort men möjligt att räkna på)
1. ≤70	
2. 71–90	
3. 91–100
:::