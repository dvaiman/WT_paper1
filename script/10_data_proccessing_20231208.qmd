...
The following is a suggested layout of the study results.
...
# income data
hpa_income <- read_csv(here("..", "..","..", "HPI (DNR XXXXXX)", "Data HPI registeruttag 2023", "SCB", "Leverans", "Leverans_20240306", "with_hpi_lopnr", "6.income_variables", "income_data.csv"))

# concecutive years of work
hpa_consq <- read_csv( here::here("..", "..","..", "HPI (DNR XXXXXX)", "Data HPI registeruttag 2023", "SCB", "Leverans", "Leverans_20240306", "with_hpi_lopnr", "5.work_duration", "workduration_with.csv"))



# join SCB, HPI and income data data
df <- hpa %>% left_join(scb, by = join_by(LopNr, Performed, Year))  %>% 
    mutate(SSYK1_scb_hpi_combined = coalesce(SSYK1, Ssyk1_hpi_combined)) %>% 
  left_join(hpa_income, by = join_by(LopNr, Performed, Year)) %>% 
  left_join(hpa_consq, by = join_by(LopNr, Year == year))

# df %>% select(LopNr, Year, Performed, consecutive_years, consecutive_years_category) %>%
#    arrange(LopNr, Performed) %>% 
#   mutate(hpa_number = row_number(), 
#          test_count = n(), .by = LopNr) %>% 
#   filter(hpa_number == 1) %>% 
#   count(consecutive_years_category)
...
PhysicalWorkSituation = case_when(
      ...
      PhysicalWorkSituation == "Stillasittande med inslag av rörligt arbete" ~ "Sedentary work with elements of mobility",
...
PhysicalWorkSituationDi = case_when(
    ...
    PhysicalWorkSituation %in% "Sedentary work with elements of activity" ~ "Sedentary work" 
    ),
 ) %>% 
  # factors
   mutate(
  ...
  SNI = factor(SNI, levels = c(
    ...
    "Activities of extraterritorial organisations and bodies"
      ...
      factor(TobaccoSmokingDi, 
             levels = c("Never", "Seldom", "Daily")),
        ExerciseDi = 
      factor(ExerciseDi, 
             levels = c("Weekly", "Seldom", "Never")),
            HealthDi = 
      factor(HealthDi, 
...
df1 |> 
 ...
 # status(.messages = "n={formatC(.count, big.mark = ',')}") %>%  
  ...
  exclude_all(
    ...
    LoneInk == 0 ~ "-No verified income, n={formatC(.excluded, big.mark = ',',format = 'fg')}",
    .headline = ""
...
val_use <- 
  colnames(
  df2 %>%
    select(HeightCM, WeightKG, BMI, BMI_cat, Astrand_MaxVO2, Astrand_rel_VO2, astr_cat,TobaccoSmokingDi, HealthDi, ExerciseDi, PhysicalWorkSituationDi))
...
val_use <- 
  colnames(
  df2 %>% select(
    Sex, AgeGroup, BirthPlace, MarriedOrPartner, EducationLevel, KommunSize, SSYK1_scb_hpi_combined_char
    )
  )
...
t2 %>% 
  mutate(Variables = case_when(
    ...
    Variables == "SSYK1_scb_hpi_combined_char" ~ "Occupation, major level, n (%)",
    TRUE ~ Variables  
  )) %>%
  slice(
    1, 2, 3, # Sex
...
Table shows relative difference/risk between SCB and HPI data in different sociodemographic categories.
...
# Define the row names including categories and subcategories
row_names <- c(
  "<b>Ownership sector (SNI)</b>",
  "1. Private",
  "2. Public",
  "<b>Economic sector (SNI)</b>",
  ...
  "3. Mining and Quarrying; Manufacturing",
  "4. Construction",
  ...
  "11. Professional, Scientific and Technical Activities; Administrative and Support Service Activities",
  "12. Public Administration and Defence",
  "13. Education",
  "14. Human Health and Social Work Activities",
  "15. Arts, Entertainment and Recreation; Other service activities",
  "<b>Number of employees in the company</b>",
  "Micro-Enterprises: 1-9 employees",
  "Small Enterprises: 10-49 employees",
  "Medium-Sized Enterprises: 50-249 employees",
  "Large Enterprises: 250+ employees",
  "<b>Multiple jobs/ economic sectors</b>",
  "1. ≥2 jobs in >1 sector",
  "2. ≥2 jobs",
  "3. 1 job",
  "<b>Staff Turnover Rate (%)</b> (PersOms_Harledd / Ast_AntalSys) only one row of data",
  "<b>Contractual relationship insecurity</b>",
  "1. Solo self-employed",
  "2. Self- and direct employment / Self-employed / Agency employed",
  "3. Directly employed",
  "<b>Contractual temporariness</b>",
  "1. Stable employment, Having the same employer for 3 years",
  "2. Unstable employment, Having the same employer for <3 years",
  "<b>Net sales % of median</b>",
  "≥200%",
  "120–199%",
  "80–119%",
  "60–79%",
  ...
  "60–79%",
  "<60%",
  "<b>Operating Margin</b> (operating profit/revenue)x100",
  "<b>Collective bargaining agreement coverage (%)</b>"
)

# Define the column names
column_names <- c("1995-1999", "2000-2004", "2005-2009", "2010-2014", "2015-2019", "2020-2023")

# Create an empty dataframe
empty_df <- tibble(Characteristic = row_names)

# Add empty columns for the time periods
for (col_name in column_names) {
  empty_df[[col_name]] <- NA
}

# View the structured table
empty_df %>% 
datatable(
  escape = FALSE,  # to render HTML
  extensions = 'Buttons',
  options = list(
    ...
    title = "Sammanställd Frekvens av ICD-koder",
        columnDefs = list(
      list(className = 'dt-body-right', targets = 3:7) # Target columns 2 to 7 -rightalign
    ),
    initComplete = JS(
      "function(settings, json) {",
      "$(this.api().table().header()).css({'font-size': '10px'});",
