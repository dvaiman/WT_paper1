...
# Function 8 
Extract income data for those who performes an HPA, income data is used to select the study sample
...
LoneInk: Likely refers to cash gross salary including various forms of cash compensation before taxes.
LonFInk: Appears to encompass both employment income and business income, including profits from self-employment or business ownership.
CSFVI: Indicates a combined or aggregated measure of earned income from possibly multiple sources or income streams.
...
output_folder <- here("..", "..","..", "HPI (DNR XXXXXX)", "Data HPI registeruttag 2023", "SCB", "Leverans", "Leverans_20240306", "with_hpi_lopnr", "6.income_variables")
...
for (year in years) {
  ...
  # Ensure only the required columns are selected/retained and add the year column
  temp_data <- temp_data %>%
    mutate(year = year) %>%
    select(LopNr, year, LoneInk, LonFInk) %>% 
...
write_csv(final_data, file.path(output_folder, "income_data.csv"))
...
# Function 7
If having a same employer for more or less than 2 years
...
summarize data into 5 year groups for those without HPI data
...
process_datasets <- function(year_group, file_paths) {
 ...
 datasets <- map_df(file_paths, ~read_csv(.x) %>%
                      select(LopNr, AgeGroup, Sex, BirthPlace, MarriedOrPartner, 
                             KommunSize, EducationLevel,Ownership_sector, Ssyk_majorlevel, IncomeLevel, 
  ...
  #  variable lists
categorical_vars <- c("AgeGroup", "Sex", "BirthPlace", "MarriedOrPartner", "KommunSize", "Ownership_sector",
...
summarize data for those with HPI data
```{r}
...
summary_files_with %>%
  ...
  # adding the 3 years or more of workplace category
  bind_rows(
  bind_rows(
 read_csv(here::here("..", "..","..", "HPI (DNR XXXXXX)", "Data HPI registeruttag 2023", "SCB", "Leverans", "Leverans_20240306", "with_hpi_lopnr", "5.work_duration", "workduration_with_summarized.csv")) %>% 
  rename( "Categories"=consecutive_years_category ,
...
all_summarized_data %>% 
 ...
 mutate(
    Variable = factor(Variable, levels = c("AgeGroup", "Sex", "BirthPlace", "EducationLevel", "IncomeLevel", "Income_Sources_Category", "Ownership_sector", "KommunSize",  "MarriedOrPartner",
    ...
    Categories = case_when(
      ...
      Variable == "Ownership_sector" ~ fct_relevel(Categories, "Offentligt", "Privat"),
...
# Ownership sector, instutution category
# "Public Govermental",
#         substr(InstKod, 3, 3) %in% c("2") ~ "Public Regional",
#         TRUE ~ "Private"
